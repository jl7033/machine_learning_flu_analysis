...
severe_flu = read_csv("severe_flu.csv") |>
  select(-id) |>
  mutate(
    race = case_match(
      race,
...
openxlsx::saveWorkbook(wb_quant,
                       here::here("summ_stats_quant.xlsx"),
                       overwrite = TRUE)
...
ggplot(flu_boost_fit, highlight = TRUE)
...
summary(flu_boost_fit$finalModel, las = 2, cBars = 19, cex.names = 0.6)
...
plot(flu_svm_linear_fit_1, transform.y = log, transform.x = log,
...
plot(flu_svm_radial_fit, transform.y = log, transform.x = log,
     color.palette = terrain.colors)
